...
const NODE_MESH_DOC = "https://github.com/AzielEliab/aziel-runtime/blob/main/docs/NODE_MESH.md";
const QNM_WP_DOC = "https://github.com/AzielEliab/aziel-runtime/blob/main/docs/designs/QNM-WP-1.0.md";
const QNM_NODE_WP = "https://github.com/AzielEliab/qnm-node/blob/main/docs/QNM-WP-1.0.md";
...
export const MESH_PRODUCT = "postking";
...
export const MESH_NOTE =
  "QNM-BUILD-1.0 + QNS-CD-1.0 (photon QNS1 packet transfer). Suite mesh default off. Live|locked|isolated counts only. No Node Gate. No public qnsd proxy. No auto-heal. Not an anonymity network. Hub cite / Worker mesh cross-map only — not a Softwares-tab product. Local qnsd is AzielEliab/qnm-node. Author: Aziel Eliab only.";
...
/** Stamp QNS-CD-1.0 so peers see the hub-cite cross-map. Does not enable mesh. */
...
export function meshPointer() {
  return {
    ...
    qns_cd_spec: QNS_CD_SPEC,
    qns_cd: QNS_CD,
    note: "PROXY to aziel-runtime /v1/mesh/* via AZIEL_RUNTIME. Not a local op. Not AnonBroadcast. Not AZMail's product-local ring. Not a Softwares-tab product. No public qnsd proxy. Post-King Chess remains asymmetric continuity chess (Node not a king). Full node process is local qnm-node/. QNS-CD-1.0 cites qnm-node + aziel-runtime (photon QNS1 packet transfer). " + MESH_NOTE,
...
export function meshOpenApiPaths() {
  ...
  for (const route of MESH_PROXY_ROUTES) {
    ...
    for (const method of route.methods) {
      entry[method] = {
        operationId: "postking_mesh_" + route.op + (method === "head" ? "_head" : "") + "_proxy",
...
async function originFetch(env, pathAndQuery, init, request) {
  ...
  headers.set("X-Aziel-Runtime-Via", "postking-download-tracker");
...
export async function handleMeshApi(request, url, env) {
  ...
  return meshJson(stampQnsCd(result.data), result.status);
}
